...
if ($this->mode == 'update') {
    ...
    if (isset($this->location_id)) {
        $rec['LOCATION_ID'] = (int)$this->location_id;
    } else {
        $rec['LOCATION_ID'] = gr('location_id', 'int');
    }
    //updating 'USER_ID' (select)
    if (isset($this->user_id)) {
        $rec['USER_ID'] = (int)$this->user_id;
    } else {
        $rec['USER_ID'] = gr('user_id','int');
    }
    //updating 'ACTION_TYPE' (select)
    $rec['ACTION_TYPE'] = gr('action_type','int');
    //updating 'SCRIPT_ID' (select)
    if (isset($this->script_id)) {
        $rec['SCRIPT_ID'] = (int)$this->script_id;
    } else {
